...
def check_sft_model(args):
    ...
    with open(log_file_path, 'a', encoding='utf-8') as log_file:
        log_file.write(f"\n{'='*40}\n")
        log_file.write(f"Testing SFT Model start time: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        ...
        for prompt in prompt_iter:
            ...
            # SFT 模型需要使用 Chat Template 进行格式化，拼装 system, user, assistant 角色
            ...
            with torch.no_grad():
                generated_ids = model.generate(
                    ...
                    repetition_penalty=1.1 # SFT模型稍微增加一点重复惩罚
                )
    ...
    print(f"\n日志已保存至: {os.path.abspath(log_file_path)}")
